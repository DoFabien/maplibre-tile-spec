...
function decodePage(
    ...
    // --- Byte container materialization (int32 words -> byte header stream) ---
    ...
    // --- Exception streams header (bitmap) + unpack exception values ---
    ...
    // --- Block unpack (packed main values + exception patching) ---
    let bytePosIn = 0;
    ...
    // --- Postconditions: packed region consumed + advance offsets ---
...
function decodeAlignedPages(
    ...
    while (outPos.get() !== finalOut) {
        const thisSize = Math.min(PAGE_SIZE, finalOut - outPos.get());
